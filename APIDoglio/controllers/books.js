...
class Books extends BaseController {
   ...
   properties calls "store" and "copies" and returns a single list, with "store" being a unique key
   */
         ...
         this.lib.db.model("Book")
            ...
            .exec((err, bookModel) => {
               if (!bookModel) {
                  ...
                  bookModel.stores = this.mergeStores(bookModel.stores, bookData.
                     stores)
               }
               bookModel.save((err, book) => {
                  if (err) return next(this.RESTError('InternalServerError',
                     err))
                  this.writeHAL(res, book)
               })
            })
